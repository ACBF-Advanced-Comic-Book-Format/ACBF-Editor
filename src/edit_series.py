...
class SeriesDialog(Gtk.Window):
    def __init__(self, parent: Gtk.Window):
        ...
        for series in self.parent.acbf_document.sequences:
            self.model.append(
                Series(name=series[0], volume=series[1], number=series[2]),
            )
        ...
        volume_column = Gtk.ColumnViewColumn(
            title="Volume Number",
        ...
        number_column = Gtk.ColumnViewColumn(
            title="Issue Number",
    ...
    def setup_volume_column(self, factory: Gtk.ListItemFactory, list_item: Gtk.ListItem) -> None:
        entry: Gtk.Entry = Gtk.Entry()
        entry.set_width_chars(5)
        entry.set_alignment(1)
        list_item.set_child(entry)

    def setup_number_column(self, factory: Gtk.ListItemFactory, list_item: Gtk.ListItem) -> None:
        entry: Gtk.Entry = Gtk.Entry()
        entry.set_width_chars(5)
        entry.set_alignment(1)
        list_item.set_child(entry)
    ...
    def text_changed(self, entry: Gtk.Entry, item: Series, attribute: str) -> None:
        # Do not trigger a model updated otherwise the text will highlight
        setattr(item, attribute, entry.get_text())
        self.set_modified()

    def on_delete_button_clicked(self, button: Gtk.Button, item: Series) -> None:
        found, position = self.model.find(item)
        if found:
            self.model.remove(position)
            self.set_modified()

    def add_series(self, button: Gtk.Button) -> None:
    ...
    def save(self) -> None:
        ...
        while i < 1000:
            ...
            self.parent.acbf_document.sequences.append(
                (row.name, row.volume, row.number),
            )
            i = i + 1
