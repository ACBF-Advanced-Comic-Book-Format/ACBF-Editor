...
class GenresDialog(Gtk.Window):
    def __init__(self, parent):
        ...
        self.model = Gio.ListStore(item_type=Genre)
    ...
    def setup_match_column(self, factory: Gtk.ListItemFactory, list_item: Gtk.ListItem):
    ...
    def bind_name_column(self, factory: Gtk.ListItemFactory, list_item: Gtk.ColumnViewCell):
        item: Genre = list_item.get_item()
        position = list_item.get_position()
    ...
    def unbind_name_column(self, factory: Gtk.ListItemFactory, list_item: Gtk.ColumnViewCell):
        # TODO Unbind click
        item: Genre = list_item.get_item()
        position = list_item.get_position()
        entry: Gtk.Label = list_item.get_child()
    ...
    def bind_match_column(self, factory: Gtk.ListItemFactory, list_item: Gtk.ListItem):
        item: Genre = list_item.get_item()
        entry: Gtk.SpinButton = list_item.get_child()
        entry.set_value(item.match)
        entry.connect("value-changed", self.match_change, item)

    def unbind_match_column(self, factory: Gtk.ListItemFactory, list_item: Gtk.ListItem):
        entry: Gtk.SpinButton = list_item.get_child()
        entry.disconnect_by_func(self.match_change)

    def genre_activate(self, widget: Gtk.CheckButton, item: Genre):
        active = widget.get_active()
        item.active = active
        self.model.sort(sort_genres)
        self.is_modified = True

    def match_change(self, widget: Gtk.SpinButton, item: Genre):
        match = widget.get_value()
        item.match = match
        self.is_modified = True

    def label_clicked(self, gesture: Gtk.GestureClick, presses: int, x: int, y: int, item: Genre, position: int, checkbox: Gtk.CheckButton):
        active = True
        if item.active:
            active = False
    ...
    def save(self) -> None:
        ...
        while i < 1000:
            ...
            if row is None:
                break
            if row.active:
                self.parent.acbf_document.genres.append((row.name.lower().replace(" ", "_"), row.match))
            i = i + 1
