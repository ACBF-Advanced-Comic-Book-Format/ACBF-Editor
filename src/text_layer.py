...
class TextLayer:
    def __init__(self, filename, page_number, acbf_document, language_layer, text_layer: Gio.ListStore, frames_layers):
        self.bg_color = '#000000'
        self.rotation = 0
        self.acbf_document = acbf_document
        self.PILBackgroundImage = Image.open(filename)
        self.PILBackgroundImageProcessed = None
        self.text_areas = text_layer #, self.references =  # acbf_document.load_page_texts(page_number, acbf_document.languages[language_layer][0])
        self.updated = False
        # print constants.FONTS_LIST
        self.normal_font = acbf_document.font_styles['normal']
        self.strong_font = acbf_document.font_styles['strong']
        self.emphasis_font = acbf_document.font_styles['emphasis']
        self.code_font = acbf_document.font_styles['code']
        self.commentary_font = acbf_document.font_styles['commentary']
        self.sign_font = acbf_document.font_styles['sign']
        self.formal_font = acbf_document.font_styles['formal']
        self.heading_font = acbf_document.font_styles['heading']
        self.letter_font = acbf_document.font_styles['letter']
        self.audio_font = acbf_document.font_styles['audio']
        self.thought_font = acbf_document.font_styles['thought']
        self.frames = frames_layers # acbf_document.load_page_frames(page_number)
    ...
    def draw_text_layer(self, *args):
        ...
        for i in range(self.text_areas.get_n_items()):
            text_area = self.text_areas.get_item(i)
            polygon = []
            ...
            if text_area.rotation == 0:
                ...
                original_polygon_size = ((original_polygon_boundaries[2] - original_polygon_boundaries[0]),
                                         (original_polygon_boundaries[3] - original_polygon_boundaries[1]))

                # move polygon to 0,0
                polygon_center_x = ((original_polygon_boundaries[2] - original_polygon_boundaries[0]) / 2) + \
                                   original_polygon_boundaries[0]
                polygon_center_y = ((original_polygon_boundaries[3] - original_polygon_boundaries[1]) / 2) + \
                ...
                # rotate polygon
                rotated_polygon = rotatePolygon(moved_polygon, text_area.rotation)

                # move polygon to image center
                polygon = []
                rotated_polygon_boundaries = get_frame_span(rotated_polygon)
                rotated_polygon_size = ((rotated_polygon_boundaries[2] - rotated_polygon_boundaries[0]),
                ...
                draw_image = Image.new('RGBA', (rotated_polygon_boundaries[2] - rotated_polygon_boundaries[0],
                                                rotated_polygon_boundaries[3] - rotated_polygon_boundaries[1]))
            ...
            while not text_fits:
                ...
                while drawing_word < len(words):
                    ...
                    while not first_word_fits:
                        ...
                        upper_right_corner_fits = point_inside_polygon(first_word_start[0] + text_size[0],
                                                                       first_word_start[1], polygon)
                        lower_left_corner_fits = point_inside_polygon(first_word_start[0],
                                                                      first_word_start[1] + text_size[1], polygon)
                        lower_right_corner_fits = point_inside_polygon(first_word_start[0] + text_size[0],
                                                                       first_word_start[1] + text_size[1], polygon)
                    ...
                    while other_word_fits and drawing_word < len(words):
                        ...
                        text_size = (chunk_size, character_height + 1)
                        upper_right_corner_fits = point_inside_polygon(current_pointer[0] + text_size[0],
                                                                       current_pointer[1], polygon)
                        lower_right_corner_fits = point_inside_polygon(current_pointer[0] + text_size[0],
                                                                       current_pointer[1] + text_size[1], polygon)
                        ...
                        if other_word_fits and upper_right_corner_fits and lower_right_corner_fits:
                            diff_ratio = (get_frame_span(polygon)[3] - (current_pointer[1] + text_size[1])) / float(
                                text_size[1])
                            if drawing_word == len(
                                    words) - 1 and diff_ratio > 1.45 and not is_formal and not is_commentary:
            ...
            if '<CODE>' in lines[0][1].upper() or text_area.type.upper() == 'CODE':
                text_areas_draw.append(
                    (character_height, lines, 'CODE', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            elif '<COMMENTARY>' in lines[0][1].upper() or text_area.type.upper() == 'COMMENTARY':
                text_areas_draw.append(
                    (character_height, lines, 'COMMENTARY', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            elif text_area.type.upper() == 'SIGN':
                text_areas_draw.append(
                    (character_height, lines, 'SIGN', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            elif text_area.type.upper() == 'FORMAL':
                text_areas_draw.append(
                    (character_height, lines, 'FORMAL', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            elif text_area.type.upper() == 'HEADING':
                text_areas_draw.append(
                    (character_height, lines, 'HEADING', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            elif text_area.type.upper() == 'LETTER':
                text_areas_draw.append(
                    (character_height, lines, 'LETTER', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            elif text_area.type.upper() == 'AUDIO':
                text_areas_draw.append(
                    (character_height, lines, 'AUDIO', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            elif text_area.type.upper() == 'THOUGHT':
                text_areas_draw.append(
                    (character_height, lines, 'THOUGHT', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))
            else:
                text_areas_draw.append(
                    (character_height, lines, 'SPEECH', text_area.rotation, text_area.polygon, text_area.type, text_area.is_inverted))

            # rotate image back to original rotation after text is drawn
            if text_area.rotation != 0:
                draw_image = draw_image.rotate(text_area.rotation, Image.BILINEAR, 1)
                ...
                self.PILBackgroundImage.paste(draw_image,
                                              (original_polygon_boundaries[0], original_polygon_boundaries[1]),
                                              draw_image)
        ...
        for text_area in text_areas_draw:
            ...
            if text_area[2] == 'SPEACH' and text_area[0] / float(statistics.median(speach_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'COMMENTARY' and text_area[0] / float(statistics.median(commentary_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'CODE' and text_area[0] / float(statistics.median(code_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'SIGN' and text_area[0] / float(statistics.median(sign_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'FORMAL' and text_area[0] / float(statistics.median(formal_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'HEADING' and text_area[0] / float(statistics.median(heading_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'LETTER' and text_area[0] / float(statistics.median(letter_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'AUDIO' and text_area[0] / float(statistics.median(audio_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            elif text_area[2] == 'THOUGHT' and text_area[0] / float(statistics.median(thought_list)) > 1.1:
                normalized_character_height = int(round(text_area[0] / 1.1, 0))
            ...
            if normalized_character_height != text_area[0]:
                ...
                for line in text_area[1]:
                    ...
                    lines.append(((line[0][0], line[0][1] - change_in_height), line[1], (
                    line[0][0] + line_length, line[0][1] + current_character_height + 1 - change_in_height)))
            else:
                for line in text_area[1]:
            ...
            if len(lines) > 0 and text_area[2] != 'FORMAL':
                ...
                vertical_move = int(((get_frame_span(polygon)[3] - get_frame_span(points)[3]) - (
                            get_frame_span(points)[1] - get_frame_span(polygon)[1])) / 2)
                ...
                if vertical_move > 0:
                    ...
                    for move in range(vertical_move, 1, -1):
                        is_inside = True
                        for line in lines:
                            if not point_inside_polygon(line[0][0],
                                                        line[0][1] + move + int(current_character_height / 5), polygon):
                                is_inside = False
                            elif not point_inside_polygon(line[2][0],
                    ...
                    if is_inside:
                        ...
                        for idx, line in enumerate(lines):
                            ...
                            while min_coordinate_set == False:
                                if (point_inside_polygon(current_coordinate,
                                                         line[0][1] + int(current_character_height / 2), polygon) and
                            ...
                            lines[idx] = ((min_coordinate + 2, line[0][1] + vertical_move - 1), line[1],
                                          (line[2][0] - (line[0][0] - min_coordinate), line[2][1] + vertical_move - 1))
            ...
            for idx, line in enumerate(lines):
                ...
                while max_coordinate_set == False:
                    if (point_inside_polygon(current_coordinate, current_pointer[1] + int(current_character_height / 2),
                                             polygon) and
                            point_inside_polygon(current_coordinate, line[2][1], polygon)):
                ...
                for chunk in tag_split:
                    ...
                    if '<INVERTED>' in chunk_upper or text_area[6]:
                        ...
                        text_area = (
                        text_area[0], text_area[1], text_area[2], text_area[3], text_area[4], text_area[5], True)
                    elif '</INVERTED>' in chunk_upper:
                        font_color = self.acbf_document.font_colors[text_area[2].lower()]
                    ...
                    if old_line != line:
                        ...
                        if is_commentary or (text_area[5].upper() == 'FORMAL' and idx + 1 == len(lines)):  # left align
                            space_between_words = self.text_width(0, ' ', font)  # draw.textsize(' ', font=font)[0]
                            ...
                            if is_last_line:
                                ...
                                justify_space = 0
                            space_between_words = self.text_width(0, ' ', font)  # draw.textsize(' ', font=font)[0] + justify_space
                        else:  # center
                            space_between_words = self.text_width(0, 'n n', font) - self.text_width(0, 'nn', font)  # draw.textsize('n n', font=font)[0] - draw.textsize('nn', font=font)[0]
                            line_length = line[2][0] - line[0][0]
                            mid_bubble_x = ((get_frame_span(text_area[4])[0] + get_frame_span(text_area[4])[
                                2]) / 2) - line_length / 2
                    ...
                    if use_small_font and current_word != '':
                        if use_subscript:
                            current_pointer = (
                            current_pointer[0], current_pointer[1] + int(current_character_height * 0.7))
                                ...
                                for idxr, reference in enumerate(self.references):
                                    if reference_id == reference[0]:
                                        rectangle = [(current_pointer[0] - 5, current_pointer[1] - 5),
                                                     (current_pointer[0] + self.text_width(0, current_word, font_small) + 5, current_pointer[1] - 5),
                                                     (current_pointer[0] + self.text_width(0, current_word, font_small) + 5,
                                                      current_pointer[1] + int(current_character_height * 0.7) + 5),
                                                     (current_pointer[0] - 5,
                                                      current_pointer[1] + int(current_character_height * 0.7) + 5)]
                                        self.references[idxr] = (reference[0], reference[1], rectangle)

                        text_size = (
                        self.text_width(0, current_word, font_small), int(current_character_height * 0.5))
                        strikethrough_rectangle = [current_pointer[0] - int(space_between_words / 2),
                                                   ...
                                                   current_pointer[1] + int(current_character_height / 2) + 1 - int(
                                                       current_character_height / 10)]
                        ...
                        if word_count > 1:
                            ...
                            one_space = self.text_width(0, current_word + ' M', font) - \
                                        self.text_width(0, current_word + 'M', font)
                        ...
                        for one_word in current_word.split(' '):
                            ...
                            word_length = max(self.text_width(0, one_word.strip(), font) + one_space,
                        ...
                        strikethrough_rectangle = [word_start[0] - int(space_between_words / 2),
                                                   ...
                                                   word_start[1] + int(current_character_height / 2) + 1 - int(
                                                       current_character_height / 10)]
    ...
    def text_width(self, chunk_size, chunk, font):
